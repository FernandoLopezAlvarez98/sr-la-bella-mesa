...
class Table
{
    ...
    /**
     * Método create simplificado para Super Admin
     */
    public function create($data)
    {
        $id = $this->generateUUID();
        
        $sql = "INSERT INTO mesa (id_mesa, id_restaurante, numero_mesa, capacidad, estado) 
                VALUES (?, ?, ?, ?, ?)";
        ...
        try {
            ...
            error_log("ERROR al crear mesa: " . $e->getMessage());
            throw new Exception('Error al crear mesa: ' . $e->getMessage());
        }
    }
    ...
    /**
     * Obtener todas las mesas con estado actual filtradas por restaurante
     */
    public function getTablesWithStatusByRestaurant($restaurantId, $date = null)
    {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        return $this->connection->fetchAll($sql, [$date, $date, $date, $date, $restaurantId]);
    }

    /**
     * Obtener todas las mesas filtradas por restaurante
     */
    public function getAllTablesByRestaurant($restaurantId)
    {
        $sql = "SELECT 
                    m.id_mesa,
                    m.numero,
    ...
    /**
     * Obtener mesas disponibles filtradas por restaurante
     */
    public function getAvailableTablesByRestaurant($restaurantId, $date, $time, $capacity)
    {
        $sql = "SELECT 
                    m.id_mesa,
                    m.numero,
        ...
        return $this->connection->fetchAll($sql, [$restaurantId, $capacity, $date, $time, $date, $time, $date, $time, $time]);
    }

    /**
     * Obtener estadísticas de ocupación filtradas por restaurante
     */
    public function getOccupancyStatsByRestaurant($restaurantId, $date = null)
    {
        if ($date === null) {
            $date = date('Y-m-d');
        }
        ...
        $sql = "SELECT 
                    ...
                    ROUND(
                        ...
                        END) * 100.0 / NULLIF(COUNT(DISTINCT m.id_mesa), 0))
                    , 2) as porcentaje_ocupacion
                FROM mesa m
                WHERE m.id_restaurante = ?";
        
        return $this->connection->fetchOne($sql, [$date, $date, $restaurantId]);
    }

    /**
     * Obtener estadísticas de mesas filtradas por restaurante
     */
    public function getTablesStatsByRestaurant($restaurantId)
    {
        $sql = "SELECT 
                    COUNT(*) as total_mesas,
                    SUM(capacidad) as capacidad_total,
